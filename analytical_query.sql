...
WITH StatisticheReparti AS (
    SELECT 
        ...
        ROUND(CAST(SUM(order_details.reordered) AS NUMERIC) / COUNT(order_details.product_id) * 100, 2) AS percentuale_riordino
    FROM departments
...
JOIN OreDiPunta ON StatisticheReparti.department_id = OreDiPunta.department_id
WHERE OreDiPunta.num_riga = 1
